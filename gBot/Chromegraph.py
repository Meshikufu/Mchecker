...
df = pd.DataFrame({'DateTime': dates, 'Price': price, 'Name': names, 'Stock': quantity})

# Create a custom hover template
hover_template = (
    "<b>%{customdata[0]}</b> | "
    "Price: %{y:$,.2f} | "
    "Stock: %{customdata[1]}<extra></extra>"
)

# Create a Plotly figure with custom hover template
fig = px.line(df, x='DateTime', y='Price', color='Name', custom_data=['Name', 'Stock'])
